...
def validar_imagen(contenido: bytes, *, content_type_cliente: str, campo: str) -> str:
    ...
    if len(contenido) == 0:
        raise ErrorValidacion(
            codigo="imagen_vacia",
            mensaje="El archivo de imagen esta vacio.",
            campo=campo,
        )
    if len(contenido) > TAMANO_MAXIMO_BYTES:
        raise ErrorValidacion(
            codigo="imagen_demasiado_grande",
            mensaje=f"La imagen excede el tamano maximo de {TAMANO_MAXIMO_BYTES // (1024 * 1024)} MB.",
            campo=campo,
        )
    ...
    if formato_real == MIME_HEIC:
        raise ErrorValidacion(
            ...
            mensaje="El formato HEIC no esta soportado. Converti la imagen a JPEG o PNG.",
            campo=campo,
        )
    ...
    try:
        ...
        raise ErrorValidacion(
            ...
            mensaje="La imagen no se pudo decodificar correctamente.",
            campo=campo,
        ) from exc
    if ancho < ANCHO_MINIMO or alto < ALTO_MINIMO:
        raise ErrorValidacion(
            codigo="imagen_resolucion_insuficiente",
            mensaje=f"Resolucion minima requerida: {ANCHO_MINIMO}x{ALTO_MINIMO} px. Imagen recibida: {ancho}x{alto} px.",
            campo=campo,
        )
    if ancho > ANCHO_MAXIMO or alto > ALTO_MAXIMO:
        raise ErrorValidacion(
            codigo="imagen_resolucion_excesiva",
            mensaje=f"Resolucion maxima permitida: {ANCHO_MAXIMO}x{ALTO_MAXIMO} px. Imagen recibida: {ancho}x{alto} px.",
            campo=campo,
        )
    return formato_real
